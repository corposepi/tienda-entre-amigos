...
self.addEventListener('fetch',e=>{
  ...
  if(req.mode==='navigate'){
    e.respondWith(fetch(req.url,{cache:'reload'}).then(r=>{const cp=r.clone();caches.open(CACHE).then(c=>c.put('./index.html',cp));return r}).catch(()=>caches.match('./index.html')));
    return;
  }
